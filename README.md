...
**Gear commands:** `get [item]` (nearest floor item), `get from corpse` / `loot from corpse` (an adjacent corpse's gear), `loot` (an adjacent corpse's gear first, else the nearest floor item), `drop <item>`, `inventory`, `examine <item>`, `equip <item>`, `use <item>`. `look` lists nearby items and corpses; stand adjacent to pick up.
